...
/// <summary>
/// <see cref="IAgentBackend"/> implementation that uses the OpenAI Chat Completions API.
...
public sealed class OpenAIBackend : IAgentBackend
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        while (true)
        {
            ...
            {
                yield return new UsageEvent("openai", totalInputTokens, totalOutputTokens);
    ...
    private static OpenAIJsonArgs ParseArgs(string json)
    {
        if (string.IsNullOrWhiteSpace(json))
            return new OpenAIJsonArgs(new Dictionary<string, JsonElement>());

        try
        {
            var dict = JsonSerializer.Deserialize<Dictionary<string, JsonElement>>(json)
                ?? new Dictionary<string, JsonElement>();
            return new OpenAIJsonArgs(dict);
        }
        catch
        {
            return new OpenAIJsonArgs(new Dictionary<string, JsonElement>());
...
/// </summary>
internal sealed class OpenAIJsonArgs : IReadOnlyDictionary<string, object?>
{
    private readonly IReadOnlyDictionary<string, JsonElement> _inner;

    internal OpenAIJsonArgs(IReadOnlyDictionary<string, JsonElement> inner) =>
        _inner = inner;
